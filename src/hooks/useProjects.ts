...
import { ProjectStateType, setProjectMessage, setProjectsStatus,  } from "../store/slices/projectsSlice"
...
export function useProjects() {
    ...
    return {state: projectsState, actions:{
        async create(data: ProjectType){
            ...
            if(projectId === undefined) {
                ...
                dispatch(setProjectMessage("Failed to create project"))
            }
        },
        async update(id: string, data: Partial<ProjectType>){
            ...
            if(isUpdated) {
                dispatch(setProjectsStatus(StatusEnum.ERROR))
                dispatch(setProjectMessage("Failed to update project"))
            }
        },
        async remove(id: string){
            ...
            if(isRemoved) {
                dispatch(setProjectsStatus(StatusEnum.ERROR))
                dispatch(setProjectMessage("Failed to remove project"))
            }
        },
    }}
}